...
[![Last Updated](https://img.shields.io/badge/Last%20Updated-April%202026-green.svg)]()

Pokemon Champions launched on April 8, 2026. This repository is the definitive data source for competitive play: every character's base stats, the complete move and ability pool, the full item list, and detailed documentation of game mechanics unique to Champions — including the SP stat distribution system that replaces the traditional EV framework.
...
|-----------|----------|-------|
| `pokemon/` | Roster, base stats, types, abilities | 258 |
| `moves/` | Move database (type, power, category, effects) | 900 |
| `abilities/` | Ability descriptions and effect data | 191 |
| `items/` | Item effects and competitive usage notes | 583 |
| `natures/` | Nature stat modifiers | 25 |
| `learnsets/` | Which characters learn which moves | 258 |
| `type-chart/` | 18x18 type effectiveness matrix | 18 |
| `meta/` | Version info and data sources | - |
| `mechanics/` | Game formula documentation and system guides | - |

The 258 playable characters include base forms, mega evolutions, and regional variants. Every form with distinct stats is represented as a separate entry.
...
learnsets = fetch("learnsets/learnsets.json")
...
### `pokemon/base-stats.json` — the stats block
...
If you have found an error — wrong stats, a move a character cannot actually learn, an ability that does not match what the game shows — please file an issue using the [data correction template](.github/ISSUE_TEMPLATE/data-correction.yml).

---

## Data Sources

- **Showdown open-source data files** — base stats, moves, abilities, items, learnsets, and type chart. Showdown's data is well-maintained and serves as the starting point for all entries.
- **Community verification** — Champions-specific corrections submitted by players with in-game evidence (screenshots, video, cross-player confirmation).
- **In-game data mining** — where available, direct extraction from game files takes precedence over all other sources.

The `meta/` directory contains version history and per-file source attribution.
