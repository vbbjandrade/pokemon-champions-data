...
export interface LearnsetDelta extends Omit<RepoLearnset, "moves"> {
...
export interface RegulationDelta {
  ...
  overrides: {
    ...
    learnsets: Record<string, LearnsetDelta | null>;
...
export function extractTextOverrides(textEntry?: any): { desc: string | null; shortDesc: string | null } {
  ...
  const championsEntry = textEntry['champions'];
  const desc = championsEntry?.desc ?? textEntry.desc ?? null;
  const shortDesc = championsEntry?.shortDesc ?? textEntry.shortDesc ?? null;
  ...
  for (const [key, entry] of Object.entries(rawPokedex)) {
    ...
    pokedex[key] = {
      ...
      baseStats: parseBaseStats(entry.baseStats),
			sources: ["smogon/pokemon-showdown"]
    };
  }
